...
class ImageRetrieval(nn.Module):
    ...
    def forward(self, query_images, query_ids, query_categories, bbox_sizes=None, bbox_centralities=None):
        ...
        if self.dataset_mode == "server":
            from aisum_database import AisumDBAdapter
            retrieved_image_urls = AisumDBAdapter.get_image_urls_for_server(self.config, result_ids)
            result_local_paths = retrieved_image_urls
        else:
        ...
        return {
            ...
            "result_local_paths": result_local_paths,
